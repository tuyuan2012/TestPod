...
+ (instancetype)sharedInstance {
    static LLRouter *router;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        router = [[LLRouter alloc] init];
    });
    return router;
}

/*
 request: scheme://[target]/[action]?[params]
 
 url sample: llScheme://targetA/actionA?id=1234
 */

- (id)performActionWithUrl:(NSURL *)url
                completion:(void (^)(NSDictionary *))completion {
    if (![url.scheme isEqualToString:@"llScheme"]) {
        //处理远程app调用404
        return @(NO);
    }
